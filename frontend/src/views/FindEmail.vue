  ...
  <div class="main-content">
    ...
    <form @submit.prevent="handleFindEmail">
      ...
      <div class="form-group">
        ...
        <input type="tel" id="phoneNumber" v-model="form.phoneNumber" placeholder="'-'를 포함한 연락처를 입력해 주세요. Ex) 010-1234-5678" required pattern="^[0-9\-+() ]+$" title="유효한 연락처를 입력해 주세요.">
      </div>
...
export default {
  ...
  methods: {
    async handleFindEmail() {
      if (this.form.nickname.trim() === '' || this.form.phoneNumber.trim() === '') {
        alert('모든 필드를 채워주세요.');
        return;
      }

      try {
        // 백엔드 API 호출: POST /api/v1/find/email
        const response = await axios.post('/api/v1/find/email', {
        ...
        if (error.response) {
          if (error.response.status === 404) {
            this.errorMessage = '해당 정보를 가진 사용자를 찾을 수 없습니다.';
          } else {
