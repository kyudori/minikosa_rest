  ...
  <div class="main-content">
    ...
    <form @submit.prevent="submitForm">
      ...
      <div class="form-group">
        ...
        <span class="file-name">{{ storePhotoName || getFileName(storePhotoPreview) || '선택된 파일 없음' }}</span>
  ...
  setup() {
    ...
    const loadStoreData = async () => {
      try {
        ...
        categoryId.value = storeData.categoryId !== undefined ? storeData.categoryId : categoryNameToId(storeData.categoryName)
        openingTime.value = storeData.openingTime ? storeData.openingTime.substring(0,5) : ''
        closingTime.value = storeData.closingTime ? storeData.closingTime.substring(0,5) : ''
    ...
    // 메뉴 삭제 처리 함수
    const onMenuDelete = async (menuId, index) => {
      try {
    ...
    const submitForm = async () => {
      ...
      try {
        // 가게 수정
        await adminStore.updateStore(storeId, storeData, storePhoto.value, storePhotoPreview.value)
        ...
        for (const menu of menuData) {
          if (menu.menuId) {
            ...
            await adminStore.updateMenu(menu.menuId, {
              menuName: menu.menuName,
              price: menu.price
            }, menu.menuPhoto, menu.menuPhotoPreview)
          } else {
            // 새로운 메뉴 생성
